...
![image-20240601160308984](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616767.png) 
...
3. **循环引用问题**：两个或多个对象相互引用，或者⼀些复杂的数据结构，如双向链表中，存在多个引⽤路径等情况下，可能会存在循环引用问题，导致资源无法
 被释放掉。
...
3. 没有`operator*`和`operator->`操作符重载，不具有一般指针的行为
...
![image-20240615124052217](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616706.png)  

![image-20240615124125185](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616800.png) 
...
- lambda 函数中，按值捕获的变量在 lambda 函数内部是**只读**的
- 需要在 lambda 函数中使用改变了捕获参数的值，但又不想将这个改变传回源捕获参数，按值传递的话需要在函数体内**复制**一次，用**mutable**就不用

![image-20240611171903964](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616631.png) 
...
![image-20240612165808170](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616803.png) 
...
![image-20240612170526683](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616846.png) 
...
- **单继承内存模型**
  ...
  ![image-20240613150949586](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616489.png) 

  ```c++
  // 如果类型 B 中出现了基类型A中没有的虚函数，新的虚函数将会被附加在虚函数表的最后，不会对与基类重合的部分造成影响。例如B中新增加了函数f2()，虚函数表变化如下：
  ```

  ![image-20240613151130318](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616420.png) 
...
- 多继承内存模型
  ...
  ![image-20240613151446359](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616587.png) 
...
- **右值引用的作用**：==主要目的就是通过延长右值的生命周期来提高程序运行效率==
...
![image-20240613121025163](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202406131211751.png)  ![image-20240613121341346](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616717.png) 

### 链接

**(1) 空间和地址重分配**：本例中`foo1.o`和`foo2.o`中均含有`.text .data`等对应段，链接时会将多个可重定位文件的相似段进行**合并**，如多个`.text`段合并为一个`.text`段, 示意图如下:

![img](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616048.jpeg)
...
CPU访问内存(拉内存数据)时，并不是逐个字节访问，而是==**以字长（word size）为单位访问**==。比如32位的CPU，字长为4字节，那么CPU访问内存的单位也是4字节。这么设计的目的，是为了**减少CPU访问内存的次数，加大CPU访问内存的吞吐量**
...
1. **不是所有的硬件平台都支持随意地址访问**，有些硬件平台 CPU 只能从指定地址处读取指定大小的数据，减少了异常抛出
...
现在考虑4字节存取粒度的处理器取int类型变量（32位系统），该处理器只能**从地址为4的倍数的内存开始读取数据**
...
![image-20240709132506997](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616767.png) 

现在有了内存对齐的，int类型数据只能存放在**按照对齐规则的内存中**，比如说0地址开始的内存。那么现在该处理器在取数据时**一次性就能将数据读出来了**，而且不需要做额外的操作，提高了效率

![image-20240709132542641](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616917.png) 

### 内存对齐规则

1.在任何一个平台下都有一个**默认对齐系数**，gcc中默认`#pragma pack(4)`，可以通过预编译命令`#pragma pack(n)，n = 1,2,4,8,16`来改变这一系数
2.结构体每个成员变量又有一个自己**独立的对齐系数**，就是成员变量类型的大小
3.实际成员变量的对齐单位是成员自己独立的对齐系数与默认对齐系数中**较小**的那个
4.每个成员变量都应该对齐到这个对齐数的整数倍处
5.结构体的总大小，应该是**实际最大对齐数的整数倍大小**，不足则补位
...
typedef struct S {
	...
	double b;
	char c;
}S; // 8 + 8 + 8 = 24， a 和 c 有补齐
```

![image-20240709133205451](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616185.png) 
...
![image-20240709133403702](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616192.png) 

## 内存泄露定义、定位及其预防方法

### 定义

在 C++ 中，内存泄漏（Memory Leak）是指程序在动态分配内存(堆区)后，由于某种原因 [1. `new/malloc` 未 `delete/free`  2. 指向动态分配内存的指针在释放内存前被**重新赋值或丢失**，导致无法访问该内存进行释放 ] **未能释放已分配的内存，导致该内存无法再被程序使用或管理的现象**。内存泄漏会导致程序的内存占用不断增加，最终可能导致程序崩溃或系统资源耗尽

### 原因

1. **未释放**: `new` 没 `delete` , `malloc` 没 `free`

   ```c++
   int fun() {
       char * pBuffer = malloc(sizeof(char)); // malloc 了没 free
...
2. **未匹配**：`new` + `free`  `new[]` + `delete`
   ...
   此处会产生内存泄漏，`new` 操作符会先通过 `operator new` 分配一块内存，然后在该块内存上调用 `placement new` 即调用 Test 的**构造函数**。而在上述代码中，只是通过 `free` 函数释放了内存，但是**没有调用 Test 的析构函数以释放 Test 的成员变量，从而引起内存泄漏**
   ...
   调用了10次构造函数，但是只调用了一次析构函数，所以引起了**内存泄漏**。这是因为调用 `delete t` 释放了通过 `operator new[]` 申请的内存，即 `malloc` 申请的内存块，且**只调用了 t[0] 对象的析构函数，t[1..9] 的析构函数并没有被调用**

3. **未虚析构**：继承需要将**父类的析构函数声明为虚函数**，`std::string`的析构函数非虚函数，故不建议继承，因为存在**内存泄漏**的风险
   ...
   int main() {
     ...
     delete base; // output: in Base::~Base  可见没有调用派生类Derived的析构函数，如果派生类中在堆上申请了资源，那么就会产生内存泄漏
     return 0;
   }
   ```

   ![image-20240709155555047](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616350.png) 
...
4. **循环引用**：
   ...
   class Controller {
    public:
     ...
     ~Controller() { std::cout << "in ~Controller" << std::endl; }
   
     class SubController { // 内部嵌套类
      public:
       SubController() = default;
   
       ~SubController() { std::cout << "in ~SubController" << std::endl; }
   
       std::shared_ptr<Controller> controller_;
       std::weak_ptr<Controller> controller_; // 用weak_ptr取代在循环引用中的一个shared_ptr,可以打破循环引用而不引发内存泄漏
     };
   ...
   int main() {
     ...
     controller->sub_controller_ = sub_controller; // 循环引用
     sub_controller->controller_ = controller;		// 循环引用
   
     std::cout << "controller use_count: " << controller.use_count() << std::endl; 		// controller use_count: 2
     std::cout << "sub_controller use_count: " << sub_controller.use_count() << std::endl; // sub_controller use_count: 2
               
     return 0;  // 没有调用Controller和SubController的析构函数
   }
   ```

   因为引用计数都是2，所以在main函数结束的时候，不会调用controller和sub_controller的析构函数，所以就出现了**内存泄漏**

### 避免

1. **避免在堆上分配**：根据具体的使用场景，如果对象可以在栈上进行分配，就在栈上进行分配，一方面栈的效率远高于堆，另一方面，还能避免内存泄漏

2. **申请释放要匹配**：a.`malloc` -> `free`  b.`new` -> `delete`  c.`new[]` -> `delete[]`

3. **避免使用裸指针，使用智能指针**

4. **使用STL中或自己实现的对象**: 如：使用`std::string` 替代`char*`, `string`类自己会进行内存管理，而且优化的相当不错

5. **RAII**: ==**谁申请，谁释放**==
   `RAII`是`Resource Acquisition is Initialization(资源获取即初始化)`的缩写，是C++语言的一种管理资源，避免泄漏的用法

   利用的就是C++**构造的对象最终会被销毁**的原则。利用C++**==对象生命周期的概念来控制程序的资源==**,比如内存,文件句柄,网络连接等。
...
1. **日志**：这种方案的核心思想，就是在每次分配内存的时候，**打印指针地址**，在释放内存的时候，**打印内存地址**，这样在程序结束的时候，通过分配和释放的差，如果**分配的条数大于释放的条数**，那么基本就能确定程序存在内存泄漏，然后根据日志进行详细分析和定位
2. **统计**：分配的时候，**统计分配次数**，在释放的时候，则**统计释放的次数**，这样在程序结束前**判断这俩值是否一致**，就能判断出是否存在内存泄漏(类似引用计数的思想)
3. **工具**：Linux --- valgrind   Windows --- umdh
4. **经验之谈**：如果我们上线后，发现程序存在内存泄漏，如果不严重的话，可以先暂时不管线上，同时进行排查定位；如果线上泄漏比较严重，那么第一时间根据实际情况来决定**是否回滚**。在定位问题点的时候，可以采用**缩小范围法**，着重分析这次新增的代码，这样能够有效缩短问题解决的时间
...
- **用途**：在堆上分配一块**指定大小的内存**，并返回指向这块内存的指针

- **返回值**：成功时返回指向已分配内存的指针(`void*`)；失败时返回 `NULL`
...
- **用途**：在堆上分配一块内存，并**初始化为零**。分配的内存大小为 `num * size`。
...
- **用途**：调整先前分配的内存块的大小，可以增加或减少内存块的大小。如果 `ptr` 为 `NULL`，则等同于 `malloc`。如果 `size` 为 0 且 `ptr` 非空，则等同于 `free`

- **返回值**：成功时返回指向已重新分配内存的指针；失败时返回 `NULL`，并且原内存块保持不变

- **特点**：可以改变已有内存块的大小，并在需要时移动内存块，重新分配成功时会**自动释放原内存**
...
- **用途**：在**栈上动态分配**一块指定大小的内存，并返回指向这块内存的指针。分配的内存会在函数返回时**自动释放**

- **特点**：分配的内存在函数返回时自动释放，不需要显式调用 `free`、不会产生内存碎片、访问受限，即只能在当前函数或者作用域内访问

```c++
void example_function(size_t num_elements) {
    int* array = (int*)alloca(num_elements * sizeof(int));
    // 使用分配的内存
}
```
...
1. 线程池：用本地套接字+epoll，epoll实现了接收方(线程池)的无锁线程安全，而线程局部存储(`std::thread_local`)实现了各个模块不同线程发任务的无锁线程安全(每个线程持有自己的客户端套接字)
2. 日志服务器：用本地套接字+epoll，epoll实现了接收方(日志服务器)的无锁线程安全，而线程局部存储(`std::thread_local`)实现了各个模块不同线程发送日志给日志服务器的无锁线程安全(每个线程持有自己的客户端套接字)、写日志用单线程，不用考虑读写锁的问题

## 模板类---泛型编程 + C++11新特性 + 多态

**==通用的==可调用对象类，用于生成==任意==函数指针与参数数量的==可调用对象== **

1.  定义可调用对象基类（为了隔绝模板的传染性**[类A中如果有一个成员是模版类B的对象，要么就指定这个成员的模版参数，要么类A也只能是模版类]**，在其他类中的可调用对象均声明为基类指针类型）
...
4. 初始化可调用对象（模版函数、**`std::placeholders`**）==注意==如果函数调用方式是**`_thiscall`**(类的非静态成员函数)，第一个参数要传**`this`**
   ...
   {
       CUseFunc useFunc;
       useFunc.SetFunc(func); // 初始化可调用对象 模板函数在使用时，编译器可以自动推导出参数类型 而模板类在实例化时需要显式指定模板参数类型
       myFunc<int(*)()> obj(func); // 模版类在实例化时需要显式指定模版参数类型
...
static void Trace(const LogInfo & info) { // 给其他非日志进程的进程和线程使用的 可以通过类名+作用域符来调用  CLoggerServer::Trace()
    ...
    if (client == -1) { // 静态的但每个线程(static thread_local)调这个函数都会构造一个新的 client 即一个线程对应一个客户端 线程局部存储
...
//LOGI<<"hello" << 10 <<"how are you"; // 流式日志在日志对象析构时通过判断 m_bool == true 来调Trace(）
...
## 客户端连接服务器(主模块) --- 单Reactor多线程 + 业务接口多态(工厂模式)
...
**功能**：网络客户端 MD5(时间，用户名，密码，盐)得到**摘要值**，再将 **时间 + 盐 + 用户名 + 数字签名** 做http请求发送给服务器 (密码没有传输)
...
| 7.硬件中断（如定时器中断、I/O 中断等）也会导致线程切换 | 操作系统响应中断后，可能会改变当前线程的运行状态，从而调度其他线程运行 |
...
| 区别         | 堆                                                           | 栈                                                           |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 分配⽅式     | 堆上的内存是**动态分配**的，程序在运行时可以根据需要分配和释放内存 | 栈则有静态分配和动态分配两种，静态分配是编译器完成的，比如局部变量的分配，而栈的动态分配则通过`alloca()`函数完成，但栈动态分配的内存由**编译器进行释放** |
| 变量⽣命周期 | 堆上的内存⽣命周期取决于程序员**⼿动分配和释放**。分配的堆内存在程序运⾏过程中⼀直存在，直到被显式释放或程序结束被操作系统回收 | 栈上的局部变量会在作用域（如函数作用域、块作用域等）结束后析构、释放内存 |
| 变量访问权限 | 变量可以在**进程范围内访问**，即进程内的所有线程都可以访问该变量 | 只能在当前函数或者作用域内进行访问，不能跨线程访问(故**==局部变量是线程安全的==**) |
| 速度         | 比栈慢，涉及 `brk()`、`mmap()`等系统调用                     | 栈上内存分配和释放，是一个进栈和出栈的过程(对于编译器只是**一个移动指针的过程**)，所以相比于堆上的内存分配，栈要快的多 |
| 大小         | 堆的大小通常比栈⼤得多，因此可以用于存储较⼤的数据结构和对象。然而，分配和释放堆内存的操作相对较慢，可能导致程序性能下降 | 栈的大小相对较⼩，适用于存储较⼩的数据结构和对象。分配和释放栈内存的操作非常快速，但栈空间有限，可能导致栈溢出错误；虽然栈的访问速度要快于堆，**==每个线程都有一个自己的栈，栈上的对象是不能跨线程访问的，这就决定了栈空间大小是有限制的==**，如果栈空间过大，那么在大型程序中几十乃至上百个线程，光栈空间就消耗了RAM，这就导致heap的可用空间变小，影响程序正常运行 |
| 内存碎片     | 频繁的内存分配和释放势必会造成内存空间的不连续，从而造成大量的碎片，使程序效率降低 | 不会产生内存碎片，内存都是连续的，申请和释放都是指令移动，类似于数据结构中的进栈和出栈 |
...
+ **底层结构**: 底层由 双向队列 实现 , 特点是 存储空间 **分段连续** （注意不是连续！！！） 
...
![image-20240702130554414](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242428.png)  

### 堆

上图中看出：堆是基于==**连续的存储空间**==实现的，所以堆是**完全二叉树**，这样才可以**通过索引来找到其父/子节点的存储空间**

以**小根堆**为例：

![image-20240702131009616](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242429.png) 
...
![image-20240704215613413](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242430.png)

- 新版 STL：可以看出在新版 STL 中，这些容器的设计都采用了 **pimpl 模式** 

![image-20240704215858802](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242431.png) 

#### vector 的内存释放问题

vector 与 deque ，其内存占用空间**只会增长，不会减小**。比如你首先分配了10,000个字节，然后 erase 掉后面 9,999 个，则虽然有效元素只有一个，但是内存占用仍为 10,000 个，所有空间在 vector 析构时回收

下面给出两种 **vector 的内存释放方式**：两者都会进行原 vector 所有元素的**拷贝与析构**，即会有**拷贝构造和析构函数的调用 **

- `clear() / resize()` + `shrink_to_fit()` **C++ 11**
- `swap()`
...
int main() {
  ...
  // 1. clear() 只会清空数据，并不会释放已申请的内存
  a.clear();
  std::cout << a.capacity() << std::endl;  // 128
  // 2. resize() 只会改变元素数量，不会释放已申请的内存
  a.resize(10);
  std::cout << a.capacity() << std::endl;  // 128
  // 3. shrink_to_fit() 可以帮助释放 vecor 中没有被使用的内存
  a.shrink_to_fit();
  std::cout << a.capacity() << std::endl;  // 10
  // 4. 使用 swap() 来构造一个相同的 vector，可以释放原 vector 所占的内存
  // https://blog.csdn.net/sukhoi27smk/article/details/27505467
  std::vector<int> b;
  b.swap(a);
  std::cout << a.capacity() << std::endl;  // 0  原 vector 所占内存在 swap() 后，全部释放
  std::cout << b.capacity() << std::endl;  // 10 新构造的没有预留空间，故大小与实际使用的相等
  return 0;
}
```

![image-20240704212745838](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242432.png) 

#### vector的扩容机制

![image-20240704212842309](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242433.png) 

![image-20240704213036514](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242434.png) 

#### vector 与 list 的对比

- 遍历两个元素数目相同的`vector`和`list`，哪个效率高？

从时间复杂度来看，都是 $O(n)$, 但是没有考虑到**缓存**问题。实际上因为 `vector` 底层采用数组存储数据，所以它的空间局部性更好，对缓存更友好（`Cache-friendly`），所以遍历 `vector` 的效率要高于遍历 `list`

#### at.() 和 [] 随机访问的区别

![image-20240709100029466](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616810.png) 

**应用场景**：

- 确定索引在有效范围内，并且对性能要求很高，可以使用 `operator[]`。

- 不确定索引是否在有效范围内，或者希望在索引超出范围时抛出异常以便进行处理，应该使用 `at()`。 
...
![image-20240704221651608](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407042242435.png) 
...
![image-20240709102555033](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616811.png) 
...
`std::deque` 的内部结构是一个==**分段的连续数组**==。它由一个**指针数组**（称为“地图”）和多个**固定大小的缓冲区**（或块）组成。每个缓冲区存储一部分元素。这种设计允许 `std::deque` 在两端高效地进行插入和删除操作，而无需像 `std::vector` 那样频繁地进行内存重新分配和移动元素

- **详细解释**：

1. **内存管理**：`std::deque` 使用多个固定大小的缓冲区，每个缓冲区包含一定数量的元素。缓冲区之间的连接由一个指针数组（地图）维护。这个指针数组存储指向每个缓冲区的指针。这样，当需要在两端插入或删除元素时，只需在适当的缓冲区中进行操作，而不需要移动其他元素

2. **访问元素**：访问 `std::deque` 中的元素时，首先通过计算索引确定元素所在的缓冲区，然后在缓冲区中定位元素。这使得 `std::deque` 能够在 `O(1)` 时间内进行随机访问

3. **双端操作**：由于 `std::deque` 在两端都有缓冲区，当在头部插入元素时，只需在头部缓冲区中插入元素，或者分配一个新的缓冲区并更新地图。同样，在尾部插入元素时，只需在尾部缓冲区中插入元素，或者分配一个新的缓冲区并更新地图。这使得双端操作的时间复杂度为 `O(1)`

#### deque 的扩容机制

**检查 buffer 容量 -> 分配新 buffer -> 检查 map 容量 -> 扩容 map(其实就是扩容 vector)**

![image-20240709103942391](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616812.png) 
...
- `clear() / resize()` + `shrink_to_fit()` **C++ 11**
- `swap()` ：与一个空的 `deque` 交换内容，这会导致原来的 `deque` 被销毁，从而释放其内存

#### stack &   queue

内部用 **deque** 支持，通过**禁用和改变某些成员方法**即可实现 **stack & queue**

也可以用 **list** 作为底层容器，但是效率没有 **deque** 高

- **stack**: `push() = push_back()` `pop() = pop_back()`
- **queue:** `push() = push_back()` `pop() = pop_front()`

![image-20240709124152387](https://496051480-1319142273.cos.ap-guangzhou.myqcloud.com/202407091616813.png) 

- 在 C++ 中，`std::stack` 和 `std::queue` 是基于**容器适配器**实现的，它们的设计目标是**提供简单的栈和队列操作接口**，而不是像 `std::vector`、`std::list` 等容器那样提供全面的迭代器支持。因此，`std::stack` 和 `std::queue` 并**不直接支持遍历操作，且不提供迭代器**
- **stack** 可以选择 **vector** 作为底层容器，但是 **queue** 不可以
- **stack** 和 **queue** 都不可以选择 **set** 或 **map** 作为底层容器
